...
def combine_frag(read1, read2, read1_out, read2_out, statfile, frag_size, linker_seq):
    ...
    for r1,r2 in zip(fq(read1), fq(read2)):  # iteration Python 3's zip function is Python 2's izip
        ...
        # remove framents/qual <20nt (if m=20)in read2
        if len(r1_fragments) == len(r1_qual):
            for x,y in zip(r1_fragments, r1_qual):
                if len(x)< m and len(y)< m:
                    r1_fragments.remove(x)
                    r1_qual.remove(y)
        else:
             print ("check the read1: {}, frag_count is not equal to qual_count \n".format(r1[0]))
                
        # remove framents/qual <20nt in read2
        if len(r2_fragments) == len(r2_qual):
            for x,y in zip(r2_fragments, r2_qual):
                if len(x)< m and len(y)< m:
                    r2_fragments.remove(x)
                    r2_qual.remove(y)
        else:
