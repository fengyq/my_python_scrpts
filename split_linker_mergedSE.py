...
# @Last Modified time:  Jun 10 22:30:48 2021
## Python version 2.7
...
def linker_remove(linker_seq, read1, read0_out, read1_out, read2_out, statfile, frag_size):
    ...
    for r1 in fq(read1):  ## iteration using the predefined function fq()
        ...
        # remove framents<20nt
        for x in r1_fragments:
            if len(x)< m:
                r1_fragments.remove(x)
        for x in r1_qual:
            if len(x)< m:
                r1_qual.remove(x)

        ## Write read into files
        #1 if one fragment >20nt in the splited read
        if len(r1_fragments)==1:
            L1=L1+1
            for line in r1:
                r0_nouse.write(line)
        ...
        #2 if multiple fragmen >20nt in the splited read
            ...
            n=0
            # Repair the fragments one by one
            for f1,f2,q1,q2 in zip(r1_fragments[:-1],r1_fragments[1:],r1_qual[:-1],r1_qual[1:]):
                n=n+1
                newname=r1[0].replace('GWNJ-0965:633:GW2007273281','',1).rstrip() + "_split" + str(n)  # rename the framents
                r1_L=[newname,f1,"+",q1]
        ...
        # other conditions
             ...
             print ("check the read {}, fragment is not equal to quality \n".format(r1[0]))
